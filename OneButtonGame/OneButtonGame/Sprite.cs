...
namespace OneButtonGame
{
    ...
    {
        //Vectors for position Direction and Orgin
        public Vector2 position, Direction, Origin;  //Origin starts at top left can be moved to center by uncommenting code in LoadContent
        public float Speed;
        ...
        public Texture2D texture;  //current Texture
        public Texture2D SpriteTexture
        {
            ...
            set
            {
                ...
                this.SpriteTextureData =
                    new Color[this.texture.Width * this.texture.Height];
                this.texture.GetData(this.SpriteTextureData);
            }
        }
        ...
        private void updateRectangeForDrawing()
        {
            ...
            rectangle.Width = (int)(texture.Width * this.Scale);
            rectangle.Height = (int)(texture.Height * this.Scale);
        }
        ...
        public virtual void SetTranformAndRect()
        {
            ...
            if (this.texture != null)
            {
                ...
                this.locationRect = CalculateBoundingRectangle(
                         new Rectangle(0, 0, (int)(this.texture.Width * Scale),
        ...
        public void DrawMarkers(SpriteBatch sb)
        {
            ...
            if (showMarkers)
            {
                ...
                //location Marker
                sb.Draw(this.SpriteMarkersTexture,
                    new Rectangle((int)this.position.X - this.SpriteMarkersTexture.Width / 2,
                        (int)this.position.Y - this.SpriteMarkersTexture.Height / 2,
                        SpriteMarkersTexture.Width, SpriteMarkersTexture.Height),
                    Color.Yellow);
        ...
        public void KeepMovingAcrossScreen(Game game)
        {
            if ((this.position.X > game.GraphicsDevice.Viewport.Width - this.texture.Width) || (this.position.X < 0))
            {
                this.Direction *= new Vector2(-1, 0);
            }
            if ((this.position.Y > game.GraphicsDevice.Viewport.Height - this.texture.Height) || (this.position.Y < 0))
            {
        ...
        {
            vp = this.Game.GraphicsDevice.Viewport;
            if ((this.position.X + this.SpriteTexture.Width) <= (0 - this.Origin.X) ||
                this.position.X >= (vp.Width - this.Origin.X) ||
                (this.position.Y + this.SpriteTexture.Height) <= (0 - this.Origin.Y) ||
                this.position.Y - this.SpriteTexture.Height >= (vp.Height - this.Origin.Y))
            {
                return true;
            }
        ...
        public virtual bool PerPixelCollisionWTansform(Sprite OtherSprite)
        {
            ...
            return IntersectPixels(this.spriteTransform, this.texture.Width,
                this.texture.Height, this.SpriteTextureData,
                OtherSprite.spriteTransform, OtherSprite.texture.Width,
                OtherSprite.texture.Height,
